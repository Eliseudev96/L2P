...
// --- NOVAS IMPORTAÇÕES PARA A AUTOMAÇÃO ---
...
// Função para pegar o passe de acesso
...
// A função que faz a leitura do Excel e atualiza o MongoDB
async function sincronizarPlanilha() {
    ...
    try {
        ...
        for (let row of excel.value) {
            const codigo = row.values[0][0]; // Coluna Projeto
            const status = row.values[0][1]; // Coluna Ativo
        ...
        console.log(`✅ Sincronização concluída! ${obrasAtualizadas} ativas mantidas/criadas | ${obrasRemovidas} inativas removidas.`);
...
// Rota Oculta: Forçar a sincronização manualmente pelo navegador
